package processing.test.cargodropnx;
...
public class CargoDropNX extends PApplet {
...
public void draw() {
  ...
  if (gameState == 4) {
    ...
    for( int i = units.size()-1; i >= 0; i--) {
      ...
      // delete
      for(PVector z : zones) {
        if(u.pos.dist(new PVector(z.x,z.y)) < 50 && PApplet.parseInt(z.z)==u.type) {
          boolean doit = false;
          if(u.type == 1 && u.vel.y > 0.5f) doit = true;
           if(u.type == 2 && u.vel.x <  0.5f) doit = true;
          if(u.type == 3 && u.vel.x < 0.5f) doit = true;
          if(u.type == 4 && u.vel.x < 0.5f) doit = true; 
          if(u.type == 5 && u.vel.y > 0.6f) doit = true; 
           if(u.type == 6 && u.vel.x < 0.5f) doit = true; 
          if(u.type == 7 && u.vel.x > 0.5f) doit = true; 
           if(u.type == 8 && u.vel.x <  0.5f) doit = true; 
...
public void sss() {
  ...
  for(int i = 0; i < max; i++) {
    ...
    u.vel = d.get();
     u.type = 1;
    if(random(1) < 0.3f) u.type = 2;
    if(random(1) < 0.3f) u.type = 3;
    
     if(random(1) < 0.3f) u.type = 4;
    if(random(1) < 0.3f) u.type = 5;
    if(random(1) < 0.3f) u.type = 6;
    if(random(1) < 0.3f) u.type = 7;
    if(random(1) < 0.3f) u.type = 8;
  ...
  for(int i = 0; i < 8; i++) {
    ...
    ///add lanes and random
     ...
     PVector z = new PVector((i+1)+random(100,580), (i+1)+random(80,400), i+1);
    zones.add(z);
  }
...
//  public int sketchHeight() { return 560; }
}
